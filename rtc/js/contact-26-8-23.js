...
function showingContactsListHtml(contacts) {
    if (contacts.length > 0) {
        ...
        contacts.map(function (cont) {
            html += `<div id="contact_list_${cont.id}" class="row" style="margin-bottom: 0px;background-color:white;">
                                  
                                     <div onclick="callingAlertModal(this)" data-id="${cont.id}" data-tel="${cont.mobile_number}" data-name="${cont.name}" class="col s8 make-call-to-phone" style="position: relative;cursor: pointer ;border-bottom: 1px solid #5c5a5a;padding-top: 0px;padding-left: 20px;padding-bottom: 0px;width:100%">
                                    <div style="font-size: 22px;color:black">${cont.name}</div>
                                    <div style="font-size: 22px;color:black" id="${cont.mobile_number}">${cont.mobile_number}</div>
                                    </div>
